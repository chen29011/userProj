...
public class TestController {
	...
	public void myLogin(@RequestParam(value = "user_id") String user_id,
		...
		resp.addCookie(new Cookie("nickname","chen29011"));
		resp.addCookie(new Cookie("img_url","http://i13.tietuku.com/e5ca267b810d722b.png"));
		resp.addCookie(new Cookie("profile_url", "http://www.chenqiong.net/my/index.html"));
		resp.addCookie(new Cookie("sign",""));
	...
	public void getUserInfo(@RequestParam(value = "callback") String callback, HttpServletRequest res,
        ...
        if(!isContains("user_id", cookies)){//此处为模拟逻辑，具体实现可以变化
            ...
            user.setImg_url((String)getCookieValue("img_url",cookies));
            user.setNickname((String)getCookieValue("nickname",cookies));
            user.setProfile_url((String)getCookieValue("profile_url",cookies));
            user.setSign((String)getCookieValue("sign",cookies));
